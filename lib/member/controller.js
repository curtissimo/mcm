...
function getMany(scene) {
  ...
  member.from(scene.chapterdb).all(function (e, members) {
    ...
    members.forEach(function (m) {
      ...
      if (m.preferences && !m.preferences['directory:show:email']) {
        m.email = false;
      }
      if (m.preferences && !m.preferences['directory:show:phone']) {
        m.phone = false;
      }
    });

    members = members.filter(function (member) {
      return (member.preferences && member.preferences['directory:show'])
